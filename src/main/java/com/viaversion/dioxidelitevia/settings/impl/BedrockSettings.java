...
import com.viaversion.dioxidelitevia.api.settings.type.ButtonSetting;
import com.viaversion.dioxidelitevia.injection.access.core.bedrock.IConfirmScreen;
import com.viaversion.dioxidelitevia.save.SaveManager;
import com.viaversion.dioxidelitevia.save.impl.AccountsSave;
...
public final class BedrockSettings extends SettingGroup {
    ...
    private final ButtonSetting clickToSetBedrockAccount = new ButtonSetting(this, Component.translatable("bedrock_settings.dioxidelitevia.click_to_set_bedrock_account"), () -> {
        ...
        public MutableComponent displayValue() {
            ...
            if (account != null && account.getMinecraftMultiplayerToken().hasValue()) {
                return Component.translatable("click_to_set_bedrock_account.dioxidelitevia.display", account.getMinecraftMultiplayerToken().getCached().getDisplayName());
            } else {
                return super.displayValue();
            }
        }
    };
    public final BooleanSetting replaceDefaultPort = new BooleanSetting(this, Component.translatable("bedrock_settings.dioxidelitevia.replace_default_port"), true);
    public final BooleanSetting experimentalFeatures = new BooleanSetting(this, Component.translatable("bedrock_settings.dioxidelitevia.experimental_features"), true);

    public BedrockSettings() {
        super(Component.translatable("setting_group_name.dioxidelitevia.bedrock"));
    }
        ...
        try {
            final BedrockAuthManager bedrockAccount = BedrockAuthManager
                ...
                .login(DeviceCodeMsaAuthService::new, (Consumer<MsaDeviceCode>) msaDeviceCode -> {
                    DioxideLiteViaScreen.setScreen(new ConfirmScreen(copyUrl -> {
                        if (copyUrl) {
                    ...
                    }, TITLE, Component.translatable("click_to_set_bedrock_account.dioxidelitevia.notice"), Component.translatable("base.dioxidelitevia.copy_link"), Component.translatable("base.dioxidelitevia.cancel")));
            ...
            DioxideLiteViaScreen.setScreen(prevScreen);
        } catch (Exception e) {
            ...
            if (client.screen instanceof SettingsScreen) { // The user might have already left the screen and joined a server
                DioxideLiteViaScreen.showErrorScreen(TITLE, e, prevScreen);
            }
        }
    }
    ...
    private static void updateLoginStatusMessage(final String stepName) {
        Minecraft.getInstance().execute(() -> {
            if (Minecraft.getInstance().screen instanceof ConfirmScreen confirmScreen) {
                ((IConfirmScreen) confirmScreen).DioxideLiteVia$updateMessage(Component.translatable("minecraftauth_library.dioxidelitevia." + stepName));
            }
        });
    }
