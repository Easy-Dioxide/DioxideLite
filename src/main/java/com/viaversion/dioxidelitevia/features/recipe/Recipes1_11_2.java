...
package com.viaversion.dioxidelitevia.features.recipe;

import com.viaversion.dioxidelitevia.features.recipe.custom.AddBannerPatternRecipe;
import com.viaversion.dioxidelitevia.features.recipe.custom.ShulkerBoxColoringRecipe;
...
public final class Recipes1_11_2 {
    ...
    public static void init() {
        ...
        final JsonArray recipes = DioxideLiteViaMappingDataLoader.INSTANCE.loadData("recipes-1.11.2.json").getAsJsonArray("");
        ...
        if (RECIPE_MANAGER == null) {
            ...
            for (int i = 0; i < LEGACY_RECIPES.size(); i++) {
                ...
                if (legacyRecipe.getB().contains(ProtocolTranslator.getTargetVersion())) {
                    final ResourceKey<Recipe<?>> key = ResourceKey.create(Registries.RECIPE, Identifier.fromNamespaceAndPath("dioxidelitevia", "recipe/" + i));
            ...
            for (CraftingRecipe specialRecipe : specialRecipes) {
                final ResourceKey<Recipe<?>> key = ResourceKey.create(Registries.RECIPE, Identifier.fromNamespaceAndPath("dioxidelitevia", "recipe/special_" + specialRecipe.getClass().getSimpleName().replace("Recipe", "").toLowerCase(Locale.ROOT) + (index == 0 ? "" : index)));
