...
package com.viaversion.dioxidelitevia.features.item.filter_creative_tabs;
...
import static com.viaversion.dioxidelitevia.features.classic.cpe_extension.CPEAdditions.EXTENDED_CLASSIC_ITEMS;
...
public final class VersionedRegistries {
    ...
    public static void init() {
        final JsonObject data = DioxideLiteViaMappingDataLoader.INSTANCE.loadData("versioned-registries.json");
    ...
    public static boolean keepItem(final Item item) {
        if (ProtocolTranslator.getTargetVersion().equals(c0_30cpe)) {
            ...
            final ExtensionProtocolMetadataStorage extensionProtocol = ((IConnection) handler.getConnection()).DioxideLiteVia$getUserConnection().get(ExtensionProtocolMetadataStorage.class);
