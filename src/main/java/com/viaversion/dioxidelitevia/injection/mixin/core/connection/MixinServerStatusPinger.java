...
public abstract class MixinServerStatusPinger {
    ...
    private Connection setForcedVersion(InetSocketAddress inetSocketAddress, EventLoopGroupHolder eventLoopGroupHolder, LocalSampleLogger localSampleLogger, Operation<Connection> original, @Local(argsOnly = true) ServerData serverInfo) {
        ...
        if (mixinServerInfo.DioxideLiteVia$forcedVersion() != null && !mixinServerInfo.DioxideLiteVia$passedDirectConnectScreen()) {
            ...
            ((ILocalSampleLogger) localSampleLogger).DioxideLiteVia$setForcedVersion(mixinServerInfo.DioxideLiteVia$forcedVersion());
            mixinServerInfo.DioxideLiteVia$passDirectConnectScreen(false);
        }
