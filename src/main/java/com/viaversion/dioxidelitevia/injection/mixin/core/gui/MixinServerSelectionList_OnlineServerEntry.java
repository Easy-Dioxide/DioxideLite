...
package com.viaversion.dioxidelitevia.injection.mixin.core.gui;
...
public abstract class MixinServerSelectionList_OnlineServerEntry {
    ...
    private void drawTranslatingState(GuiGraphicsExtractor instance, Component component, int x, int y, Operation<Void> original) {
        ...
        if (GeneralSettings.INSTANCE.showAdvertisedServerVersion.getValue()) {
            final ProtocolVersion version = ((IServerData) serverData).DioxideLiteVia$translatingVersion();
            if (version != null) {
                tooltips.add(Component.translatable("base.dioxidelitevia.via_translates_to", version.getName() + " (" + version.getOriginalVersion() + ")"));
                tooltips.add(Component.translatable("base.dioxidelitevia.server_version", serverData.version.getString() + " (" + serverData.protocol + ")"));
            }
        }
