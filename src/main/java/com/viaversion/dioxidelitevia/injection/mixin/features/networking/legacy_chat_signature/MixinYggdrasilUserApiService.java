...
import com.viaversion.dioxidelitevia.features.networking.legacy_chat_signature.KeyPairResponse1_19_0;
...
public abstract class MixinYggdrasilUserApiService {
    ...
    public KeyPairResponse getKeyPair() {
        ...
        if (response.publicKeySignature() != null && response.publicKeySignature().array().length != 0) {
            ((IProfilePublicKey_Data) (Object) keyPairResponse).dioxidelitevia$setLegacyPublicKeySignature(response.publicKeySignature().array());
        } else {
            DioxideLiteViaImpl.INSTANCE.getLogger().error("Could not get legacy public key signature. 1.19.0 with secure-profiles enabled will not work!");
        }
