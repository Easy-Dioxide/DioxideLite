...
package com.viaversion.dioxidelitevia.protocoltranslator;
...
import com.viaversion.dioxidelitevia.DioxideLiteViaImpl;
import com.viaversion.dioxidelitevia.injection.access.core.IConnection;
import com.viaversion.dioxidelitevia.protocoltranslator.impl.command.DioxideLiteViaCommandHandler;
import com.viaversion.dioxidelitevia.protocoltranslator.impl.platform.DioxideLiteViaViaLegacyPlatform;
import com.viaversion.dioxidelitevia.protocoltranslator.impl.platform.DioxideLiteViaViaVersionPlatform;
import com.viaversion.dioxidelitevia.protocoltranslator.impl.viaversion.DioxideLiteViaPlatformLoader;
import com.viaversion.dioxidelitevia.protocoltranslator.netty.NoReadFlowControlHandler;
import com.viaversion.dioxidelitevia.protocoltranslator.netty.DioxideLiteViaDecoder;
import com.viaversion.dioxidelitevia.protocoltranslator.protocol.DioxideLiteViaProtocol;
import com.viaversion.dioxidelitevia.protocoltranslator.util.NoPacketSendChannel;
...
public final class ProtocolTranslator {
     ...
     */
    public static final AttributeKey<Connection> CLIENT_CONNECTION_ATTRIBUTE_KEY = AttributeKey.newInstance("dioxidelitevia-clientconnection");

    /**
     * Stores the target version ViaVersion is translating to. This will be always set, even if the {@link #NATIVE_VERSION} is set.
     */
    public static final AttributeKey<ProtocolVersion> TARGET_VERSION_ATTRIBUTE_KEY = AttributeKey.newInstance("dioxidelitevia-targetversion");
    ...
    /**
     * Injects the DioxideLiteVia pipeline with all ViaVersion elements into a Minecraft pipeline
     *
    ...
    public static void injectViaPipeline(final Connection connection, final Channel channel) {
        ...
        final ProtocolVersion serverVersion = mixinClientConnection.DioxideLiteVia$getTargetVersion();
        ...
        mixinClientConnection.DioxideLiteVia$setUserConnection(user);

        final ChannelPipeline pipeline = channel.pipeline();

        // ViaVersion
        pipeline.addBefore(HandlerNames.INBOUND_CONFIG, ViaDecodeHandler.NAME, new DioxideLiteViaDecoder(user));
        ...
        user.getProtocolInfo().getPipeline().add(DioxideLiteViaProtocol.INSTANCE);
    }
    ...
    public static ProtocolVersion getTargetVersion(final Channel channel) {
        if (!channel.hasAttr(TARGET_VERSION_ATTRIBUTE_KEY)) {
            throw new IllegalStateException("DioxideLiteVia has not injected into that channel yet!");
        }
    ...
    public static void setTargetVersion(final ProtocolVersion newVersion, final boolean revertOnDisconnect) {
        ...
        if (oldVersion != newVersion) {
            ...
            DioxideLiteViaImpl.CHANGE_PROTOCOL_VERSION.invoker().onChangeProtocolVersion(oldVersion, targetVersion);
        }
    }
    ...
    public static UserConnection getPlayNetworkUserConnection() {
        ...
        return ((IConnection) handler.getConnection()).DioxideLiteVia$getUserConnection();
    }
    ...
    public static CompletableFuture<Void> init(final Path path) {
        ...
        // Register command callback for /dioxidelitevia
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            final DioxideLiteViaCommandHandler commandHandler = (DioxideLiteViaCommandHandler) Via.getManager().getCommandHandler();
            ...
            dispatcher.register(LiteralArgumentBuilder.<FabricClientCommandSource>literal("dioxidelitevia").then(executor).executes(commandHandler::execute));
        });

        return CompletableFuture.runAsync(() -> {
            // Load ViaVersion and register all platforms and their components
            ViaManagerImpl.initAndLoad(
                new DioxideLiteViaViaVersionPlatform(path.toFile()),
                new NoopInjector(),
                new DioxideLiteViaCommandHandler(),
                new DioxideLiteViaPlatformLoader(),
                () -> {
                    new ViaBackwardsPlatformImpl();
                    new DioxideLiteViaViaLegacyPlatform();
            ...
            DioxideLiteViaProtocol.INSTANCE.initialize();
