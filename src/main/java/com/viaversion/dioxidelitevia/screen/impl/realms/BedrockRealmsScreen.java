...
package com.viaversion.dioxidelitevia.screen.impl.realms;

import com.viaversion.dioxidelitevia.DioxideLiteViaImpl;
import com.viaversion.dioxidelitevia.util.bedrock.NetherNetJsonRpcAddress;
import com.viaversion.dioxidelitevia.save.SaveManager;
import com.viaversion.dioxidelitevia.screen.DioxideLiteViaList;
import com.viaversion.dioxidelitevia.screen.DioxideLiteViaListEntry;
import com.viaversion.dioxidelitevia.screen.DioxideLiteViaScreen;
import com.viaversion.dioxidelitevia.util.network.ConnectionUtil;
...
public final class BedrockRealmsScreen extends DioxideLiteViaScreen {
    ...
    public BedrockRealmsScreen() {
        super(Component.translatable("screen.dioxidelitevia.bedrock_realms"), true);
    }
    ...
    protected void init() {
        ...
        setupSubtitle(Component.translatable("bedrock_realms.dioxidelitevia.availability_check"));
    ...
    private void loadRealms() {
        ...
        if (account == null) { // Just in case...
            setupSubtitle(Component.translatable("bedrock_realms.dioxidelitevia.warning"));
            return;
        }
        ...
        service.isCompatibleAsync().thenAccept(state -> {
            if (state) {
                ...
                setupSubtitle(Component.translatable("bedrock_realms.dioxidelitevia.unavailable"));
            }
    ...
    private Void error(final String message, final Throwable throwable) {
        setupSubtitle(Component.translatable("bedrock_realms.dioxidelitevia.error"));
        DioxideLiteViaImpl.INSTANCE.getLogger().log(Level.ERROR, message, throwable);
        return null;
    }
    ...
    private void createView() {
        if (!this.realmsServers.isEmpty()) {
            ...
            setupSubtitle(Component.translatable("bedrock_realms.dioxidelitevia.no_worlds"));
        }
        ...
        this.addRenderableWidget(joinButton = Button.builder(Component.translatable("bedrock_realms.dioxidelitevia.join"), button -> {
            final SlotEntry entry = (SlotEntry) slotList.getFocused();
            if (entry.realmsServer.isExpired()) {
                setupSubtitle(Component.translatable("bedrock_realms.dioxidelitevia.expired"));
                return;
            } else if (!entry.realmsServer.isCompatible()) {
                setupSubtitle(Component.translatable("bedrock_realms.dioxidelitevia.incompatible"));
                return;
            }
            ...
            try {
                ...
                if (server.getNetworkProtocol().equalsIgnoreCase(RealmsJoinInformation.PROTOCOL_DEFAULT)) {
                    ...
                    setupSubtitle(Component.translatable("bedrock_realms.dioxidelitevia.unsupported_protocol", server.getNetworkProtocol()));
                }
        ...
        xPos += 115 + 5;
        this.addRenderableWidget(leaveButton = Button.builder(Component.translatable("bedrock_realms.dioxidelitevia.leave"), button -> {
            final SlotEntry entry = (SlotEntry) slotList.getFocused();
        ...
        xPos += 115 + 5;
        this.addRenderableWidget(Button.builder(Component.translatable("bedrock_realms.dioxidelitevia.invite"), button -> {
    ...
    public final class SlotList extends DioxideLiteViaList {
    ...
    public final class SlotEntry extends DioxideLiteViaListEntry {
