...
package com.viaversion.setsunavia.features.recipe;

import com.viaversion.setsunavia.features.recipe.custom.AddBannerPatternRecipe;
import com.viaversion.setsunavia.features.recipe.custom.ShulkerBoxColoringRecipe;
        ...
        if (RECIPE_MANAGER == null) {
            ...
            for (int i = 0; i < LEGACY_RECIPES.size(); i++) {
                ...
                if (legacyRecipe.getB().contains(ProtocolTranslator.getTargetVersion())) {
                    final ResourceKey<Recipe<?>> key = ResourceKey.create(Registries.RECIPE, Identifier.fromNamespaceAndPath("setsunavia", "recipe/" + i));
            ...
            for (CraftingRecipe specialRecipe : specialRecipes) {
                final ResourceKey<Recipe<?>> key = ResourceKey.create(Registries.RECIPE, Identifier.fromNamespaceAndPath("setsunavia", "recipe/special_" + specialRecipe.getClass().getSimpleName().replace("Recipe", "").toLowerCase(Locale.ROOT) + (index == 0 ? "" : index)));
