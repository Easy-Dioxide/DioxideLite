...
public final class NameTags extends Module {
    ...
    /** Returns the current rendered tag width in logical pixels, including optional team flag. */
    public float getTagBoxWidth(Player player) {
        if (player == null) return 0.0F;
        String username = player.getName().getString();
        String name = username + LegendSuffixUtil.getPlainSuffix(username);
        String health = String.format(Locale.ROOT, "[%.1f HP]", getDisplayHealth(player));
        float flagWidth = getTabFlag(player) == null ? 0.0F : FLAG_SIZE + FLAG_GAP;
        return flagWidth + SkijaUi.textWidth(name) + 4.0F + SkijaUi.textWidth(health) + 6.0F;
    }
