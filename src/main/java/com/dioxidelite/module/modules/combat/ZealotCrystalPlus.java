...
// ---------------------------------------------------------------------------
// 移植来源：DioxideLite（上游开源版）com/dioxidelite/module/modules/combat/ZealotCrystalPlus.java
...
import com.dioxidelite.util.player.EnchantmentUtils;
...
public final class ZealotCrystalPlus extends Module {
    ...
    @Override
    public String id() {
        return "zealot_crystal_plus";
    }

    // General
    private final BooleanSetting players = boolSetting("Players", true);
    private final BooleanSetting mobs = boolSetting("Mobs", false);
    private final BooleanSetting animals = boolSetting("Animals", false);
    ...
    private final BooleanSetting eatingPause = boolSetting("Eating Pause", false);
    ...
    private final BooleanSetting forcePlaceWhileSwording = boolSetting("Force Place While Swording", false);

    // Calculation
    private final BooleanSetting assumeInstantMine = boolSetting("Assume Instant Mine", true);
    ...
    private final BooleanSetting motionPredict = boolSetting("Motion Predict", true);
    private final IntSetting predictTicks = intSetting("Predict Ticks", 8, 0, 20, 1, motionPredict::get);
    private final EnumSetting<DamagePriority> damagePriority = enumSetting("Damage Priority", DamagePriority.Efficient);
    private final EnumSetting<ArmorMode> armorMode = enumSetting("Armor Mode", ArmorMode.None);
    private final BooleanSetting lethalOverride = boolSetting("Lethal Override", true);
    private final DoubleSetting lethalThresholdAddition = doubleSetting("Lethal Threshold Addition", 0.5, -5.0, 5.0, 0.1, lethalOverride::get);
    private final DoubleSetting lethalMaxSelfDamage = doubleSetting("Lethal Max Self Damage", 16.0, 0.0, 20.0, 0.25, lethalOverride::get);
    ...
    private final BooleanSetting spamPlace = boolSetting("Spam Place", false);
    private final EnumSetting<SwitchMode> placeSwitchMode = enumSetting("Place Switch Mode", SwitchMode.Off);
    private final BooleanSetting placeSwing = boolSetting("Place Swing", false);
    ...
    private final BooleanSetting bbtt = boolSetting("2B2T", false);
    private final IntSetting bbttFactor = intSetting("2B2T Factor", 200, 0, 1000, 25, bbtt::get);
    private final EnumSetting<BreakMode> packetBreak = enumSetting("Packet Break", BreakMode.Target, () -> !bbtt.get());
    private final IntSetting ownTimeout = intSetting("Own Timeout", 100, 0, 2000, 25,
            () -> breakMode.get() == BreakMode.Own || packetBreak.get() == BreakMode.Own);
    ...
    private final BooleanSetting renderTargetDamage = boolSetting("Target Damage", true);
    private final BooleanSetting renderSelfDamage = boolSetting("Self Damage", true);
    ...
    protected void onEnable() {
        placeTimer.reset();
        breakTimer.reset();
        snapshotTimer.setMs(updateDelay.get().longValue());
    ...
    private void onTick(PlayerTickEvent.Pre event) {
        ...
        if (breakMode.get() != BreakMode.Off && breakTimer.passedMillis(breakDelay.get()) && actionBreak != null) {
            acted = breakDirect(actionBreak);
        }

        PlaceInfo actionPlace = getActionPlaceInfo();
        if (!acted && placeMode.get() != PlaceMode.Off && placeTimer.passedMillis(placeDelay.get()) && actionPlace != null && shouldAttemptPlace(actionPlace)) {
    ...
    private void onRender3D(Render3DEvent event) {
        ...
        float moveDelta = toDelta(renderMoveStartTime, movingLength.get());
        ...
        float fadeDelta = toDelta(renderFadeStartTime, fadeLength.get());
        ...
        Color base = renderColor.get();
        Color filled = new Color(base.getRed(), base.getGreen(), base.getBlue(), Math.clamp((int) (filledAlpha.get() * renderScale), 0, 255));
        Color outline = new Color(base.getRed(), base.getGreen(), base.getBlue(), Math.clamp((int) (outlineAlpha.get() * renderScale), 0, 255));

        if (filledAlpha.get() > 0) {
            Render3DUtils.drawFilledBox(box, filled);
        }
        if (outlineAlpha.get() > 0) {
            Render3DUtils.drawOutlineBox(event.getPoseStack(), box, outline.getRGB(), outlineWidth.get().floatValue());
        }
    ...
    private void onRender2D(Render2DEvent event) {
        if (noPlayer() || renderPrevPos == null || renderCurrentPos == null) return;
        if (!renderTargetDamage.get() && !renderSelfDamage.get()) return;

        float moveDelta = toDelta(renderMoveStartTime, movingLength.get());
        Vec3 renderPos = renderPrevPos.add(renderCurrentPos.subtract(renderPrevPos).scale(easeOutQuart(moveDelta)));
        Vector2f screenPos = projectToScreen(renderPos);
        if (screenPos == null) return;

        StringBuilder label = new StringBuilder();
        if (renderTargetDamage.get()) {
            label.append(String.format(Locale.ROOT, "%.1f", renderDamage));
        }
        if (renderSelfDamage.get()) {
            if (!label.isEmpty()) label.append('/');
            label.append(String.format(Locale.ROOT, "%.1f", renderSelfDamageValue));
        }
        if (label.isEmpty()) return;

        String text = label.toString();
        float height = 10.0F;
        float width = SkijaUi.textWidth(text);
        int alpha = Mth.clamp((int) (220 * renderScale), 0, 255);
        int color = new Color(255, 255, 255, alpha).getRGB();
        Canvas canvas = event.canvas();
        SkijaUi.text(canvas, text, screenPos.x - width * 0.5F, screenPos.y - height * 0.5F, height, color);
    }

    @Override
    public String getInfo() {
        return switch (hudInfo.get()) {
            case Off -> "";
    ...
    private SnapshotData captureSnapshotIfNeeded() {
        if (!snapshotTimer.passedMillis(updateDelay.get()) && latestSnapshot != null) {
    ...
    private SnapshotData captureSnapshot() {
        ...
        SettingsSnapshot settings = new SettingsSnapshot(
                globalDelay.get(),
                noSuicide.get().floatValue(),
                placeMaxSelfDamage.get().floatValue(),
                breakMaxSelfDamage.get().floatValue(),
                placeMinDamage.get().floatValue(),
                breakMinDamage.get().floatValue(),
                placeBalance.get().floatValue(),
                breakBalance.get().floatValue(),
                forcePlaceMinDamage.get().floatValue(),
                forcePlaceBalance.get().floatValue(),
                lethalOverride.get(),
                lethalThresholdAddition.get().floatValue(),
                lethalMaxSelfDamage.get().floatValue(),
                safeMaxTargetDamageReduction.get().floatValue(),
                safeMinSelfDamageReduction.get().floatValue(),
                collidingCrystalExtraSelfDamageThreshold.get().floatValue(),
                damagePriority.get(),
                armorMode.get(),
                placeSideBypass.get(),
                packetPlace.get(),
        ...
        ResistantBlockCache resistantBlocks = new ResistantBlockCache(mc.level, assumeInstantMine.get());
    ...
    private SelfSnapshot captureSelfSnapshot(Player player, ArmorMode armorMode) {
    ...
    private List<TargetSnapshot> captureTargets(ArmorMode armorMode) {
        if (mc.player == null || mc.level == null) return List.of();

        int ticks = motionPredict.get() ? predictTicks.get() : 0;
    ...
    private TargetSnapshot buildTargetSnapshot(LivingEntity entity, int ticks, ArmorMode armorMode) {
    ...
    private List<CrystalSnapshot> captureCrystals() {
        ...
        for (Entity entity : mc.level.entitiesForRendering()) {
            ...
            boolean breakable = !bbtt.get() || current - getSpawnTime(crystal) >= bbttFactor.get();
        ...
        for (int x = minX; x <= maxX; x++) {
            for (int y = minY; y <= maxY; y++) {
                for (int z = minZ; z <= maxZ; z++) {
                    ...
                    if (isResistantState(state) && (!assumeInstantMine.get() || !FastBreak.INSTANCE.isInstantMining(pos))) {
    ...
    private BreakPlan evaluateBreak(SnapshotData snapshot, PlaceInfo placeInfo, boolean requireRotation) {
        ...
        List<CrystalSnapshot> crystalList = snapshot.crystals().stream()
                ...
                .filter(crystal -> !requireRotation || checkCrystalRotation(crystal.pos(), breakRotationRange.get(), snapshot.self().currentRotation()))
                .toList();
    ...
    private PlaceInfo getValidPlaceInfo(PlaceInfo placeInfo, boolean requirePlaceable) {
        if (placeInfo == null || noPlayer() || mc.player == null || mc.level == null) return null;
        if (placeDistanceSq(mc.player, placeInfo.hitVec().x, placeInfo.hitVec().y, placeInfo.hitVec().z) > placeRange.get() * placeRange.get()) {
            return null;
        }
    ...
    private BreakPlan getValidBreakPlan(BreakPlan breakPlan) {
        if (breakPlan == null || noPlayer() || mc.level == null) return null;
    ...
    private BreakPlan getActionBreakPlan() {
        ...
        if (fallback != null && checkCrystalRotation(fallback.pos(), breakRotationRange.get())) {
            return fallback;
        }
    ...
    private boolean placeDirect(PlaceInfo placeInfo, boolean ignoreTimer) {
        ...
        if (!ignoreTimer && !placeTimer.passedMillis(placeDelay.get())) return false;
        ...
        RotationManager.INSTANCE.setRotations(placeInfo.rotation(), getRotationSpeed(), null, Priority.High, () -> {
            ...
            if (result.consumesAction()) {
                ...
                placedPosMap.put(placeInfo.blockPos().asLong(), System.currentTimeMillis() + ownTimeout.get());
                placeTimer.reset();
    ...
    private boolean breakDirect(BreakPlan breakPlan) {
        ...
        if (placeSwitchMode.get() != SwitchMode.Ghost
                && antiWeakness.get() != SwitchMode.Ghost
                && System.currentTimeMillis() - lastSwapTime < swapDelay.get() * 50L) {
            return false;
        }

        if (mc.player == null) return false;
        if (mc.player.hasEffect(MobEffects.WEAKNESS) && !isHoldingTool()) {
            switch (antiWeakness.get()) {
                case Off -> {
                    return false;
                }
                case Legit, Ghost -> {
                    ...
                    InvUtils.swap(weaponSlot, antiWeakness.get() == SwitchMode.Ghost);
                    lastSwapTime = System.currentTimeMillis();
                }
            }
        }
        ...
        RotationManager.INSTANCE.setRotations(RotationUtils.calculate(breakPlan.pos()), getRotationSpeed(), null, Priority.High, () -> {
            ...
            if (packetPlace.get().onBreak && placeInfo != null && crystalPlaceBoxIntersects(placeInfo.blockPos(), currentCrystal.getBoundingBox())) {
    ...
    private void handleSpawnPacket(ClientboundAddEntityPacket packet) {
        ...
        if (snapshot == null || snapshot.targets().isEmpty() || bbtt.get()) return;

        Vec3 crystalPos = new Vec3(packet.getX(), packet.getY(), packet.getZ());
        if (!checkBreakRange(crystalPos) || !checkCrystalRotation(crystalPos, breakRotationRange.get())) return;
        PlaceInfo placeInfo = getActionPlaceInfo();
        if (placeInfo == null) {
        ...
        boolean shouldBreak = switch (packetBreak.get()) {
    ...
    private boolean shouldForcePlace(SelfSnapshot self, TargetSnapshot targetInfo) {
        return (!forcePlaceWhileSwording.get() || !self.swording())
                && (targetInfo.totalHealth() <= forcePlaceHealth.get()
                || targetInfo.speed() >= forcePlaceMotion.get()
                || targetInfo.minArmorRate() <= forcePlaceArmorRate.get());
    }
    ...
    private List<BlockPos> buildRawPosList() {
        if (noPlayer()) return List.of();

        List<BlockPos> list = new ArrayList<>();
        double range = placeRange.get();
        double rangeSq = range * range;
        double wallRangeSq = wallRange.get() * wallRange.get();
    ...
    private List<Entity> getCollidingEntities() {
        ...
        boolean single = placeMode.get() == PlaceMode.Single;
    ...
    private boolean checkPlaceRotation(BlockPos pos) {
        if (placeRotationRange.get() <= 0.0) return true;
    ...
    private boolean checkPlaceRotation(BlockPos pos, Rot2f currentRotation) {
        if (placeRotationRange.get() <= 0.0) return true;
        return getRotationDelta(currentRotation, RotationUtils.calculate(getCrystalPos(pos))) <= placeRotationRange.get();
    }
    ...
    private boolean checkBreakRange(Vec3 crystalPos) {
        ...
        Vec3 eyePos = mc.player.getEyePosition();
        return eyePos.distanceToSqr(crystalPos) <= wallRange.get() * wallRange.get()
    ...
    private double placeDistanceSq(Entity entity, double x, double y, double z) {
        return placeRangeMode.get() == RangeMode.Feet ? entity.distanceToSqr(x, y, z) : eyeDistanceSq(entity, x, y, z);
    }

    private double breakDistanceSq(Entity entity, double x, double y, double z) {
        return breakRangeMode.get() == RangeMode.Feet ? entity.distanceToSqr(x, y, z) : eyeDistanceSq(entity, x, y, z);
    }
    ...
    private double getRotationSpeed() {
        return Math.max(0.1, yawSpeed.get() / 18.0);
    }
    ...
    private boolean isEatingPaused() {
        if (!eatingPause.get() || mc.player == null) {
            return false;
        }
    ...
    private void updateExplosionSamples() {
        if (!explosionSampleTimer.passedMillis(250)) {
            return;
        }
    ...
    private void renderTargetPredictions(Render3DEvent event) {
        if (renderPredict.get() == RenderPredictMode.Off) return;
        ...
        for (TargetSnapshot targetInfo : snapshot.targets()) {
            if (renderPredict.get() == RenderPredictMode.Single && targetInfo.entity() != focus) {
                continue;
            }
    ...
    private BooleanSetting boolSetting(String name, boolean defaultValue) {
        return add(new BooleanSetting(name, defaultValue));
    }

    private IntSetting intSetting(String name, int defaultValue, int min, int max, int step) {
        return add(new IntSetting(name, defaultValue, min, max, step));
    }

    private IntSetting intSetting(String name, int defaultValue, int min, int max, int step,
                                  BooleanSupplier visibility) {
        return add(new IntSetting(name, defaultValue, min, max, step).visibleWhen(visibility));
    }

    private DoubleSetting doubleSetting(String name, double defaultValue, double min, double max,
                                        double step) {
        return add(new DoubleSetting(name, defaultValue, min, max, step));
    }

    private DoubleSetting doubleSetting(String name, double defaultValue, double min, double max,
                                        double step, BooleanSupplier visibility) {
        return add(new DoubleSetting(name, defaultValue, min, max, step).visibleWhen(visibility));
    }

    private <E extends Enum<E>> EnumSetting<E> enumSetting(String name, E defaultValue) {
        return add(new EnumSetting<>(name, defaultValue));
    }

    private <E extends Enum<E>> EnumSetting<E> enumSetting(String name, E defaultValue,
                                                           BooleanSupplier visibility) {
        return add(new EnumSetting<>(name, defaultValue).visibleWhen(visibility));
    }

    private ColorSetting colorSetting(String name, Color defaultValue, boolean allowAlpha) {
        return add(new ColorSetting(name, defaultValue, allowAlpha));
    }
    ...
    private static final class ResistantBlockCache {
        ...
        private boolean isResistant(long posLong) {
            ...
            if (level != null) {
                ...
                resistant = isResistantStateStatic(state) && (!assumeInstantMine || !FastBreak.INSTANCE.isInstantMining(pos));
            }
    ...
    private static final class DamageReductionData {
        ...
        private static DamageReductionData fromEntity(LivingEntity entity, ArmorMode armorMode) {
