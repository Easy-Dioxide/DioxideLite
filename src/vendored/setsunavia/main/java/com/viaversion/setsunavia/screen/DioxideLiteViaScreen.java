...
package com.viaversion.setsunavia.screen;
...
public class DioxideLiteViaScreen extends Screen {
    ...
    public void addRefreshButton(final Runnable click) {
        this.addRenderableWidget(Button.builder(Component.translatable("base.setsunavia.refresh"), button -> {
            click.run();
    ...
    public static void showErrorScreen(final Component title, final Throwable throwable, final Screen next) {
        ...
        final Minecraft client = Minecraft.getInstance();
        client.execute(() -> client.setScreen(new AlertScreen(() -> client.setScreen(next), title, Component.translatable("base.setsunavia.something_went_wrong").append("\n" + throwable.getMessage()), Component.translatable("base.setsunavia.cancel"), false)));
    }
