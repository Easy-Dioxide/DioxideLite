...
import com.viaversion.setsunavia.injection.access.core.bedrock.IConfirmScreen;
import com.viaversion.setsunavia.save.SaveManager;
import com.viaversion.setsunavia.save.impl.AccountsSave;
...
public final class BedrockSettings extends SettingGroup {
    ...
    private final ButtonSetting clickToSetBedrockAccount = new ButtonSetting(this, Component.translatable("bedrock_settings.setsunavia.click_to_set_bedrock_account"), () -> {
        ...
        public MutableComponent displayValue() {
            ...
            if (account != null && account.getMinecraftMultiplayerToken().hasValue()) {
                return Component.translatable("click_to_set_bedrock_account.setsunavia.display", account.getMinecraftMultiplayerToken().getCached().getDisplayName());
            } else {
                return super.displayValue();
            }
        }
    };
    public final BooleanSetting replaceDefaultPort = new BooleanSetting(this, Component.translatable("bedrock_settings.setsunavia.replace_default_port"), true);
    public final BooleanSetting experimentalFeatures = new BooleanSetting(this, Component.translatable("bedrock_settings.setsunavia.experimental_features"), true);

    public BedrockSettings() {
        super(Component.translatable("setting_group_name.setsunavia.bedrock"));
    }
        ...
        try {
            final BedrockAuthManager bedrockAccount = BedrockAuthManager
                ...
                .login(DeviceCodeMsaAuthService::new, (Consumer<MsaDeviceCode>) msaDeviceCode -> {
                    ...
                    }, TITLE, Component.translatable("click_to_set_bedrock_account.setsunavia.notice"), Component.translatable("base.setsunavia.copy_link"), Component.translatable("base.setsunavia.cancel")));
    ...
    private static void updateLoginStatusMessage(final String stepName) {
        Minecraft.getInstance().execute(() -> {
            if (Minecraft.getInstance().screen instanceof ConfirmScreen confirmScreen) {
                ((IConfirmScreen) confirmScreen).DioxideLiteVia$updateMessage(Component.translatable("minecraftauth_library.setsunavia." + stepName));
            }
        });
    }
