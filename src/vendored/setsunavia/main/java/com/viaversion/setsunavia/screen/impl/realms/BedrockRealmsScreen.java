...
import com.viaversion.setsunavia.util.bedrock.NetherNetJsonRpcAddress;
...
import com.viaversion.setsunavia.util.network.ConnectionUtil;
...
public final class BedrockRealmsScreen extends DioxideLiteViaScreen {
    ...
    public BedrockRealmsScreen() {
        super(Component.translatable("screen.setsunavia.bedrock_realms"), true);
    }
    ...
    protected void init() {
        ...
        setupSubtitle(Component.translatable("bedrock_realms.setsunavia.availability_check"));
    ...
    private void loadRealms() {
        ...
        if (account == null) { // Just in case...
            setupSubtitle(Component.translatable("bedrock_realms.setsunavia.warning"));
            return;
        }
        ...
        service.isCompatibleAsync().thenAccept(state -> {
            if (state) {
                ...
                setupSubtitle(Component.translatable("bedrock_realms.setsunavia.unavailable"));
            }
    ...
    private Void error(final String message, final Throwable throwable) {
        setupSubtitle(Component.translatable("bedrock_realms.setsunavia.error"));
    ...
    private void createView() {
        if (!this.realmsServers.isEmpty()) {
            ...
            setupSubtitle(Component.translatable("bedrock_realms.setsunavia.no_worlds"));
        }
        ...
        this.addRenderableWidget(joinButton = Button.builder(Component.translatable("bedrock_realms.setsunavia.join"), button -> {
            final SlotEntry entry = (SlotEntry) slotList.getFocused();
            if (entry.realmsServer.isExpired()) {
                setupSubtitle(Component.translatable("bedrock_realms.setsunavia.expired"));
                return;
            } else if (!entry.realmsServer.isCompatible()) {
                setupSubtitle(Component.translatable("bedrock_realms.setsunavia.incompatible"));
                return;
            }
            ...
            try {
                ...
                if (server.getNetworkProtocol().equalsIgnoreCase(RealmsJoinInformation.PROTOCOL_DEFAULT)) {
                    ...
                    setupSubtitle(Component.translatable("bedrock_realms.setsunavia.unsupported_protocol", server.getNetworkProtocol()));
                }
        ...
        xPos += 115 + 5;
        this.addRenderableWidget(leaveButton = Button.builder(Component.translatable("bedrock_realms.setsunavia.leave"), button -> {
            final SlotEntry entry = (SlotEntry) slotList.getFocused();
        ...
        xPos += 115 + 5;
        this.addRenderableWidget(Button.builder(Component.translatable("bedrock_realms.setsunavia.invite"), button -> {
