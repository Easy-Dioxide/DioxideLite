...
public final class MainUIShader {
    ...
    public void render(GuiGraphics graphics, double mouseX, double mouseY) {
        ...
        // [v1.8 CHANGE] glGetError() is a synchronisation point: it can force the driver to
        // drain the command queue every frame. The original called it purely to log a
        // message, so it is now sampled only for the first frames and then skipped.
        if (shaderErrorChecks < SHADER_ERROR_CHECK_LIMIT) {
            shaderErrorChecks++;
    ...
    // [v1.8 ADDITION] bounds how often glGetError() may stall the pipeline in render().
    private static final int SHADER_ERROR_CHECK_LIMIT = 60;
    private int shaderErrorChecks = 0;
