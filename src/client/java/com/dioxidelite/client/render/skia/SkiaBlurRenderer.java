...
public final class SkiaBlurRenderer {
    ...
    private boolean ensureFrameCapture(Minecraft client, DirectContext context, int sourceFramebufferId) {
        ...
        try {
            ...
            // [v1.8 CHANGE] removed glFlush(). GL commands are already ordered on this
            // context, so the blit completes before Skia samples the texture. The flush only
            // forced an extra command submission per blurred element, per frame.
            // If a driver ever shows a torn glass panel, re-add glFlush() here.
        } finally {
    ...
    // [v1.8 CHANGE] was private. SkiaRenderer now needs the same id when it draws the GUI
    // straight into the render target, so this is shared instead of duplicated.
    public static int mainFramebufferId(Minecraft client) {
