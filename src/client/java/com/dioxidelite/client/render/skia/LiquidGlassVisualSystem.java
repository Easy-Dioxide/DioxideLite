...
public final class LiquidGlassVisualSystem {
    ...
    private static int blurBudget = 4;
    ...
    public static void renderSurface(GuiGraphics graphics, float x, float y, float w, float h,
        ...
        // Blur must be composited before the Skia overlay starts drawing.
        // Keeping the two passes separate avoids nesting two Skia GL contexts
        // against the same framebuffer. The blur renderer now reuses one GPU
        // capture texture instead of allocating a texture/FBO for every widget.
        boolean blurred = false;
        if (enabled() && blurBudget > 0) {
            blurBudget--;
            blurred = SkiaBlurRenderer.getInstance().render(
                    mc, x, y, w, h, radius, 0x18D9ECFF, Config.glassBlur * 0.85f);
        }
        ...
        try {
            if (blurred) {
                LiquidGlassRenderer.drawSurface(canvas, null, x, y, w, h, radius,
                        Math.min(1f, alpha));
            } else {
    ...
    private static void drawLightweightGlass(Canvas c, float x, float y, float w, float h,
        ...
        fill.setColor(withAlpha(0x101720, .72f * alpha));
        c.drawRRect(RRect.makeXYWH(x, y, w, h, radius), fill);
        fill.setColor(withAlpha(0xD9ECFF, .055f * Config.glassHighlight * alpha));
