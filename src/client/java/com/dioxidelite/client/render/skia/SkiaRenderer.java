...
public class SkiaRenderer {
    ...
    private static final Identifier TEXTURE_ID = Identifier.fromNamespaceAndPath("dioxide_lite", "skia_frame");
    private static final Identifier REGION_TEXTURE_ID = Identifier.fromNamespaceAndPath("dioxide_lite", "skia_region");
    ...
    // [v1.8 CHANGE] Was: private static final boolean USE_GL_BACKEND_FOR_FRAME = false;
    //
    // That constant selected between the two rendering backends this file already contains:
    //
    //   false -> CPU: Skija rasterises the whole GUI into a Surface.makeRaster, then the full
    //            frame is read back with peekPixels and uploaded to a DynamicTexture
    //            (writeToTexture), and finally blitted. Cost scales with window resolution.
    //   true  -> GPU: SkiaGlBackend wraps the current render target and Skija draws straight
    //            into it. No CPU raster, no readback, no full-frame upload.
    //
    // It is now a runtime setting so it can be flipped from the Theme page without a rebuild.
    private static boolean useGpuFrame() {
        return Config.gpuFrameRendering;
    }
    ...
    public static Canvas begin() {
        if (useGpuFrame()) {
            ensureNativeLoaded();
            // [v1.8 CHANGE] Was GL_BACKEND.begin(), which targets framebuffer 0. Skija has to
            // draw into the render target Minecraft is actually presenting, otherwise the GUI
            // lands on the wrong buffer (or on the window backbuffer instead of the render
            // target). SkiaBlurRenderer resolves the same id for its framebuffer backend.
            Canvas gpuCanvas = GL_BACKEND.begin(
                    SkiaBlurRenderer.mainFramebufferId(Minecraft.getInstance()));
            if (gpuCanvas == null) return null;
            // [v1.8 ADDITION] The CPU branch used to be the only place that set this. Anything
            // calling SkiaRenderer.getScale() would otherwise see a stale value on the GPU path.
            currentScale = (float) Minecraft.getInstance().getWindow().getGuiScale();
            drawing = true;
        ...
        if (pw != lastPixelW || ph != lastPixelH || surface == null || dynamicTexture == null) {
            ...
            dynamicTexture = new DynamicTexture("dioxide_lite:skia_frame", pw, ph, false);
    ...
    public static Canvas beginRegion(int x, int y, int w, int h) {
        ...
        if (regionSurface == null || regionTexture == null || regionPixelW > regionCapacityPixelW || regionPixelH > regionCapacityPixelH) {
            ...
            regionTexture = new DynamicTexture("dioxide_lite:skia_region", newPixelW, newPixelH, false);
    ...
    public static boolean hasFrameCache() {
        if (useGpuFrame()) return GL_BACKEND.hasSurface();
    ...
    // [v1.8 CHANGE] The original allocated a fresh native Pixmap on every upload, i.e. once
    // per redraw, then freed it. One instance is reused instead.
    private static Pixmap uploadPixmap;

    private static boolean uploadSurface(Surface sourceSurface, DynamicTexture targetTexture, int uploadW, int uploadH) {
        if (uploadPixmap == null) uploadPixmap = new Pixmap();
        Pixmap pixmap = uploadPixmap;
        try {
            ...
            // [v1.8 CHANGE] was: pixmap.close();  -- the Pixmap is now reused across frames.
        }
    }
    ...
    public static void destroy() {
        ...
        destroySurface();
        destroyRegionSurface();
        // [v1.8 ADDITION] release the reused Pixmap together with the surfaces.
