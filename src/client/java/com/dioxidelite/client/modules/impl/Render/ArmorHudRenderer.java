...
public class ArmorHudRenderer {
    ...
    private static final Identifier[] CARD_TEXTURE_IDS = new Identifier[] {
            Identifier.fromNamespaceAndPath("dioxide_lite", "armor_hud_head"),
            Identifier.fromNamespaceAndPath("dioxide_lite", "armor_hud_chest"),
            Identifier.fromNamespaceAndPath("dioxide_lite", "armor_hud_legs"),
            Identifier.fromNamespaceAndPath("dioxide_lite", "armor_hud_feet")
    };
    ...
    private void renderNew(GuiGraphics graphics, Minecraft client, ArmorEntry[] entries, int screenW, int screenH) {
        ...
        if (Config.liquidGlassAllVisuals) LiquidGlassVisualSystem.renderHudDock(graphics, getRenderX(screenW)-4f, getRenderY(screenH)-4f, getEditWidth()+8f, getEditHeight()+8f, .70f);
    ...
    private void renderCardTexture(Minecraft client, CardTexture cardTexture, ItemStack stack, float guiScale) {
        ...
        if (cardTexture.surface == null || cardTexture.dynamicTexture == null || cardTexture.textureW != targetW || cardTexture.textureH != targetH) {
            ...
            cardTexture.dynamicTexture = new DynamicTexture("dioxide_lite:" + cardTexture.textureId.getPath(), targetW, targetH, false);
