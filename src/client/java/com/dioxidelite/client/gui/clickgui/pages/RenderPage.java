...
import com.dioxidelite.client.gui.clickgui.widget.*;
...
public class RenderPage extends BasePage {
    public RenderPage() {
        modules.add(new SettingModule(
                ...
                UiText.t("一键启用游戏内全局液态玻璃材质（主菜单除外）", "One-click global Liquid Glass material for in-game visuals (main menu excluded)"),
                new SettingToggle(() -> Config.liquidGlassAllVisuals, v -> { Config.liquidGlassAllVisuals = v; Config.save(); }))
                .addSub(UiText.t("透明度", "Opacity"), UiText.t("玻璃基础透明度", "Base material opacity"),
                        new SettingSlider(20, 90, "%.0f%%", () -> (double) Config.glassOpacity * 100, v -> { Config.glassOpacity = v.floatValue() / 100f; Config.save(); }))
                .addSub(UiText.t("模糊", "Blur"), UiText.t("局部背景模糊强度", "Local background blur strength"),
                        new SettingSlider(0, 200, "%.0f%%", () -> (double) Config.glassBlur * 100, v -> { Config.glassBlur = v.floatValue() / 100f; Config.save(); }))
                .addSub(UiText.t("边缘高光", "Highlight"), UiText.t("玻璃边缘光学高光", "Optical edge highlight"),
                        new SettingSlider(0, 100, "%.0f%%", () -> (double) Config.glassHighlight * 100, v -> { Config.glassHighlight = v.floatValue() / 100f; Config.save(); })));

        modules.add(new SettingModule(
                UiText.t("DioxideLite Signature HUD", "DioxideLite Signature HUD"),
                UiText.t("参考端的 HUD 信息组件，全部为展示功能；可逐项关闭", "Reference-inspired HUD information components; presentation-only and individually switchable"),
                null)
                .addSub(UiText.t("FPS", "FPS"), UiText.t("显示帧率", "Show frames per second"), new SettingToggle(() -> Config.visualFps, v -> { Config.visualFps = v; Config.save(); }))
                .addSub(UiText.t("BPS", "BPS"), UiText.t("显示移动速度", "Show movement speed"), new SettingToggle(() -> Config.visualBps, v -> { Config.visualBps = v; Config.save(); }))
                .addSub(UiText.t("坐标", "Coordinates"), UiText.t("显示 XYZ 坐标", "Show XYZ coordinates"), new SettingToggle(() -> Config.visualCoordinates, v -> { Config.visualCoordinates = v; Config.save(); }))
                .addSub(UiText.t("会话信息", "Session Info"), UiText.t("显示当前世界/服务器", "Show the current world/server"), new SettingToggle(() -> Config.visualSession, v -> { Config.visualSession = v; Config.save(); }))
                .addSub(UiText.t("按键提示", "Keybind Overlay"), UiText.t("显示常用按键提示", "Show common keybind hints"), new SettingToggle(() -> Config.visualKeybinds, v -> { Config.visualKeybinds = v; Config.save(); }))
                .addSub(UiText.t("物品栏信息", "Inventory HUD"), UiText.t("显示当前选中栏位", "Show selected inventory slot"), new SettingToggle(() -> Config.visualInventory, v -> { Config.visualInventory = v; Config.save(); }))
                .addSub(UiText.t("记分板外壳", "Scoreboard HUD"), UiText.t("显示轻量信息外壳", "Show a lightweight scoreboard shell"), new SettingToggle(() -> Config.visualScoreboard, v -> { Config.visualScoreboard = v; Config.save(); })));

        modules.add(new SettingModule(
                UiText.t("极简视觉层", "DioxideLite Minimal Visual Layer"),
                UiText.t("独立控制视觉 HUD；不包含 Combat 自动化或数据包功能", "Independent presentation HUD controls; no combat automation or packet features"),
                null)
                .addSub(UiText.t("水印", "Watermark"), UiText.t("显示 DioxideLite 品牌标识", "Show the DioxideLite brand mark"),
                        new SettingToggle(() -> Config.visualWatermark, v -> { Config.visualWatermark = v; Config.save(); }))
                .addSub(UiText.t("功能列表", "Array List"), UiText.t("显示当前开启的视觉组件", "Show active visual components"),
                        new SettingToggle(() -> Config.visualArrayList, v -> { Config.visualArrayList = v; Config.save(); }))
                .addSub(UiText.t("信息 HUD", "Info HUD"), UiText.t("显示坐标、速度与 FPS", "Show coordinates, speed and FPS"),
                        new SettingToggle(() -> Config.visualInfoHud, v -> { Config.visualInfoHud = v; Config.save(); }))
                .addSub(UiText.t("指南针", "Compass"), UiText.t("顶部显示动态方向指示", "Show a compact dynamic direction indicator"),
                        new SettingToggle(() -> Config.visualCompass, v -> { Config.visualCompass = v; Config.save(); }))
                .addSub(UiText.t("极简准星", "Clean Crosshair"), UiText.t("使用 DioxideLite 极简准星", "Use the DioxideLite minimal crosshair"),
                        new SettingToggle(() -> Config.visualCrosshair, v -> { Config.visualCrosshair = v; Config.save(); }))
                .addSub(UiText.t("环境粒子", "Ambient Particles"), UiText.t("显示低成本装饰性微粒，性能模式自动关闭", "Show low-cost decorative particles; disabled in performance mode"),
                        new SettingToggle(() -> Config.visualParticles, v -> { Config.visualParticles = v; Config.save(); })));
