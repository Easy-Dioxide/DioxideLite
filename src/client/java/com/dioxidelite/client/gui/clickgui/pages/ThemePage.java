...
public class ThemePage extends BasePage {
    public ThemePage() {
        ...
        // [v1.8 ADDITION] Signature brand mark controls. These three settings are the only
        // knobs SignatureLogo reads, so glow and transparency stay adjustable in game.
        modules.add(new SettingModule(
                UiText.t("Signature 标志", "Signature Mark"),
                UiText.t("Signature 径向菜单中心的品牌标志；发光强度与透明度均可调",
                        "Brand mark in the centre of the Signature radial menu; glow strength and opacity are adjustable"),
                new SettingToggle(() -> Config.signatureLogoEnabled,
                        v -> { Config.signatureLogoEnabled = v; Config.save(); }))
                .addSub(UiText.t("透明度", "Opacity"),
                        UiText.t("标志整体透明度", "Overall mark transparency"),
                        new SettingSlider(0, 100, "%.0f%%",
                                () -> (double) Config.signatureLogoOpacity * 100,
                                v -> { Config.signatureLogoOpacity = v.floatValue() / 100f; Config.save(); }))
                .addSub(UiText.t("发光强度", "Glow"),
                        UiText.t("标志背后的光晕强度；设为 0 可完全关闭光晕",
                                "Halo strength behind the mark; set to 0 to disable the glow entirely"),
                        new SettingSlider(0, 100, "%.0f%%",
                                () -> (double) Config.signatureLogoGlow * 100,
                                v -> { Config.signatureLogoGlow = v.floatValue() / 100f; Config.save(); })));

        // [v1.8 ADDITION] Rendering backend switch. GPU direct rendering removes the full-frame
        // readback and the full-frame texture upload that the CPU raster path performs on every
        // redraw. Turn it off if the GUI ever renders into the wrong buffer on a given driver.
        modules.add(new SettingModule(
                UiText.t("渲染后端", "Render Backend"),
                UiText.t("Skija 的 GPU 直绘与 CPU 光栅之间切换；GPU 直绘省去整帧回读与整帧上传",
                        "Switch between Skija GPU direct rendering and CPU raster; GPU direct removes the full-frame readback and upload"),
                new SettingToggle(() -> Config.gpuFrameRendering,
                        v -> { Config.gpuFrameRendering = v; Config.save(); })));
    }
