...
void main() {
    ...
    ivec2 maxTexel = ivec2(view_res) - ivec2(1);
    texel = clamp(texel, ivec2(0), maxTexel);
    float depth = texelFetch(MainDepthSampler, texel, 0).x;

    float dilatedDepth = depth;
    dilatedDepth = min(dilatedDepth, texelFetch(MainDepthSampler, clamp(texel + ivec2( 1,  0), ivec2(0), maxTexel), 0).x);
    dilatedDepth = min(dilatedDepth, texelFetch(MainDepthSampler, clamp(texel + ivec2(-1,  0), ivec2(0), maxTexel), 0).x);
    dilatedDepth = min(dilatedDepth, texelFetch(MainDepthSampler, clamp(texel + ivec2( 0,  1), ivec2(0), maxTexel), 0).x);
    dilatedDepth = min(dilatedDepth, texelFetch(MainDepthSampler, clamp(texel + ivec2( 0, -1), ivec2(0), maxTexel), 0).x);
